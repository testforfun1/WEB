...
会话管理与验证机制有着紧密相连的关系.会话认证：对于用户正在访问的页面，服务器将会判断会话是否已认证，并且用户是否有权限访问。

认证成功的会话管理一般分为三个部分：会话生成、会话维持、会话销毁。
会话生成：指用户通过服务器端认证以后，服务器会创建一个随机会话标识来维持当前用户和服务器之间的连接。
会话维持：指在会话生成阶段完成后，使用会话标识来维护当前用户与服务器的连接，以及验证会话连接合法性的过程。
会话销毁：指当客户端与服务器主动断开连接或者客户端长时间没有活动的情况下，服务器端删除会话标识、释放会话信息等过程。

在对会话进行管理的过程中，如果会话认证，生成、维持、销毁阶段设计不当，极有可能给攻击者机会伪造成合法用户进入系统窃取或者修改机密信息，更有可能会对系统造成灾难性的破坏，所以安全的会话管理设计是非常重要的。
